 ...
 */
if ( !class_exists('Wvp_Vendor_Product_Metabox') ) {
	
	class Wvp_Vendor_Product_Metabox {
		...
		public function __construct() {
			...
			if ( isset($settings['vendor_role']) && 'single' != $settings['vendor_role'] ) { 
				...
				add_action('wp_ajax_retrieve_vendor_multiuser_pricing', array($this, 'retrieve_vendor_multiuser_pricing' ));
				add_action('save_post_product', array($this, 'save_vendor_metabox_data'), 10, 1);
			}
		}
		public function add_multiuser_role_metabox() {
			add_meta_box( 'vendor-portal-pro-multiuser', esc_html__('Vendor Multi User Pricing', 'woocommerce-vendor-portal'), array($this, 'vendor_multi_user_pricing_callback'), 'product', 'normal', 'high' 
			);
		}
		public function vendor_multi_user_pricing_callback() {
			global $post, $product; 
			wp_nonce_field('wvp_vendor_multi_user', 'wvp_vendor_multi_user');
			?>
			<div class="" id="vendor-multiuser-pricing">
				...
				<div class="vendor_container"></div>
			</div>
			<?php
		}
		public function retrieve_vendor_multiuser_pricing() {
			
			check_ajax_referer( 'wvp_vendor_portal', 'security' );
			...
			$data=get_post_meta($product_id, 'vendor_multi_user_pricing', true);
			$vendor_type = get_post_meta($product_id, '_wvp_vendor_type', true);
			$vendor_price = get_post_meta($product_id, '_wvp_vendor_amount', true);
			$roles=get_terms(
			...
			$rolehtml='';
			if ( 'variable' == $ptype ) {
				$rolehtml = $this->get_variable_product_vendor( $product_id, $roles );
				$allowed_html = array(
					'input'	=> array(
								'name'  => array(),
								'value' => array(),
					...
					<table class="vendor_portal">
						...
						foreach ( $roles as $key => $role ) {
							...
							<tr>
								...
								<td>
									<select class="widefat" name="discount_type_<?php esc_attr_e($role->term_id); ?>" value="<?php esc_attr_e($vendor_type); ?>">
		...
		public function get_variable_product_vendor ( $product_id, $roles ) {
			if ( !empty($product_id) && !empty($roles) ) {
				...
				if ( !empty($variables) ) { 
					...
					<table class="vendor_portal">
						<tr>
							...
							<tr>
								...
								<td>     
									<div class="wvp-variable">
										<?php 
										foreach ( $variables as $key ) {
											$variation_id = $key['variation_id'];
											if ( isset($data[$role->term_id][$variation_id]) ) {
												$vendor_price = $data[$role->term_id][$variation_id]['vendorprice'];
												$qty=$data[$role->term_id][$variation_id]['qty'];
											} else {
												$vendor_price = get_post_meta($key['variation_id'], '_regular_price', true);
												$qty=1;
											}
											$regular_price = get_post_meta($key['variation_id'], '_regular_price', true);
											$vendor_field_name = 'vendorprice_' . esc_attr($role->term_id) . '_' . esc_attr($key['variation_id']);
											...
											<div class="variable-item">
												...
												<input type="text" name="<?php esc_attr_e($vendor_field_name); ?>" value="<?php esc_attr_e($vendor_price); ?>"/>
												<label>
					...
					<input type="hidden" name="vendor_product_type" value="variable">
					<?php 
				} else { 
		...
		public function save_vendor_metabox_data ( $post_id ) {
			// Autosave
			...
			if ( !isset($_POST['wvp_vendor_multi_user']) || !wp_verify_nonce( wc_clean($_POST['wvp_vendor_multi_user']), 'wvp_vendor_multi_user') ) {
				return;
			}
			$roles=get_terms(
				'vendor_user_roles', array(
			...
			$data=array();
			if ( !empty($roles) ) {
				if ( isset($_POST['product-type'] ) && isset($_POST['vendor_product_type']) && 'variable' == wc_clean($_POST['vendor_product_type']) ) {
					...
					if ( !empty($variables) ) {
						foreach ( $variables as $variable ) {
							...
							foreach ( $roles as $key => $role ) {
								...
								if ( isset( $_POST['vendorprice_' . $role->term_id . '_' . $variation_id]) ) {
									$data[$role->term_id][$variation_id]['vendorprice'] = is_numeric( wc_clean($_POST['vendorprice_' . $role->term_id . '_' . $variation_id]) ) ? wc_clean($_POST['vendorprice_' . $role->term_id . '_' . $variation_id]) : '';
									$vary[$role->term_id][$variation_id]['vendorprice']= is_numeric( wc_clean($_POST['vendorprice_' . $role->term_id . '_' . $variation_id]) ) ? wc_clean($_POST['vendorprice_' . $role->term_id . '_' . $variation_id]) : '';
								}
							...
							update_post_meta($variation_id, 'vendor_multi_user_pricing', $vary);
						}
						update_post_meta($post_id, 'vendor_multi_user_pricing', $data);
					}
				} else {
					foreach ( $roles as $key => $role ) {
						if ( !isset($_POST['role_' . $role->term_id]) ) {
			...
			if ( isset($_POST['vendor_product_visibility_multi']) ) {
				update_post_meta( $post_id, 'vendor_product_visibility_multi', (array) wc_clean($_POST['vendor_product_visibility_multi']) );
			} else {
				update_post_meta( $post_id, 'vendor_product_visibility_multi', '' );
			} // ends version 1.3.0
			update_post_meta($post_id, 'vendor_multi_user_pricing', $data);
			update_post_meta($post_id, 'vendor_multi_user_pricing', $data);
		}
	}
	new Wvp_Vendor_Product_Metabox();
}
